...
namespace ComixLog.Services
{
    public class ContainersService
    {
        ...
        public ContainersService(IOptions<ComixLogDatabaseSettings> ComixLogDatabaseSettings)
        {
            ...
            _containersCollection = mongoDatabase.GetCollection<Container>(ComixLogDatabaseSettings.Value.ContainersCollectionName);
        }

        // Torne os métodos virtuais
        public virtual async Task<List<Container>> GetAsync() =>
            await _containersCollection.Find(_ => true).ToListAsync();

        public virtual async Task<Container> GetAsync(string id) =>
            await _containersCollection.Find(x => x.Id == id).FirstOrDefaultAsync();

        public virtual async Task CreateAsync(Container newContainer) =>
            await _containersCollection.InsertOneAsync(newContainer);

        public virtual async Task UpdateAsync(string id, Container updatedContainer) =>
